...
export function ProductGrid({ products, categories }: { products: ProductDoc[]; categories: ProductCategoryDoc[] }) {
      ...
      {sortedCategories.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {sortedCategories.map((c) => (
            <CategoryPill key={c.id} label={c.name} active={categoryId === c.id} onClick={() => setCategoryId(c.id)} />
          ))}
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3 pb-4">
          {filtered.map((product) => (
            <ProductCard key={product.id} product={product} inCart={lines[product.id]?.quantity ?? 0} onTap={() => add(product)} />
          ))}
        </div>
      )}
    </div>
    ...
    >
      ...
      <div className="p-2.5">
        ...
        <div className="mt-0.5 flex items-center justify-between">
          <span className="text-[15px] font-black text-brand-700 dark:text-brand-400">{formatYen(product.price)}</span>
          {!soldOut && (
