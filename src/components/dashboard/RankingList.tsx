        ...
        <ol className="space-y-2.5">
          {ranking.map((r, i) => (
            <li key={r.productId} className="flex items-center gap-3">
              ...
              <span className="min-w-0 flex-1 truncate text-[13px] font-bold text-neutral-700 dark:text-neutral-200">{r.name}</span>
              <span className="flex-shrink-0 text-xs font-bold text-neutral-400">{r.qty}個</span>
              <span className="flex-shrink-0 text-[13px] font-black text-neutral-800 dark:text-neutral-100">{formatYen(r.revenue)}</span>
            </li>
          ))}
        </ol>
      )}
...
export function StockAlerts({ lowStock, soldOut }: { lowStock: DashboardStats['lowStock']; soldOut: DashboardStats['soldOut'] }) {
