...
export function HistoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.categoryId !== 'all') list = list.filter((s) => s.items.some((i) => i.categoryId === filters.categoryId));
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (s) => s.items.some((i) => i.productName.toLowerCase().includes(q)) || s.staffName.toLowerCase().includes(q),
      );
    }
