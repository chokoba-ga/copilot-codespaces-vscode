...
-- 新規セッション（匿名サインイン含む）作成時に自動でprofileを作成する。
-- ログイン不要版のため、既定では誰でも全機能を使える admin として作成する。
...
-- ----------------------------------------------------------------------------
-- events / categories / products / sales / expenses / audit_logs / staff_roster
-- ----------------------------------------------------------------------------
...
-- 事前登録の「よく使う名前」一覧（イベントをまたいだ共有リスト）
