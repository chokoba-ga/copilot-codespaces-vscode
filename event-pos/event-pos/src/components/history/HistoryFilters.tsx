  ...
  return (
    <div className="grid grid-cols-2 gap-2.5">
      <SelectField
        label="イベント日"
        value={value.dayId}
        onChange={(e) => onChange({ ...value, dayId: e.target.value })}
      >
      ...
      <SelectField
        label="状態"
        value={value.status}
        onChange={(e) => onChange({ ...value, status: e.target.value as HistoryStatusFilter })}
      >
        <option value="all">すべて</option>
      ...
      <SelectField
        label="並び替え"
        value={value.sort}
        onChange={(e) => onChange({ ...value, sort: e.target.value as HistorySort })}
      >
