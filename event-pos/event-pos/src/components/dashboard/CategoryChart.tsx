          ...
          <ul className="min-w-0 flex-1 space-y-1.5">
            {filtered.slice(0, 6).map((d, i) => (
              <li key={d.categoryId} className="flex items-center gap-2 text-xs">
                ...
                />
                <span className="min-w-0 flex-1 truncate font-bold text-neutral-600 dark:text-neutral-300">
                  {d.name}
                </span>
                <span className="flex-shrink-0 font-black text-neutral-800 dark:text-neutral-100">
                  {formatYen(d.revenue)}
                </span>
              </li>
            ))}
